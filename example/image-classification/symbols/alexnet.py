...
def get_symbol(num_classes, dtype='float32', **kwargs):
    ...
    s1 = mx.sym.BatchNorm(pool1)
    # stage 2
    conv2 = mx.sym.Convolution(name='conv2',
        data=s1, kernel=(5, 5), pad=(2, 2), num_filter=256)
    ...
    s2 = mx.sym.BatchNorm(pool2)
    # stage 3
    conv3 = mx.sym.Convolution(name='conv3',
        data=s2, kernel=(3, 3), pad=(1, 1), num_filter=384)
    ...
    s3 = mx.sym.BatchNorm(pool3)
    # stage 4
    flatten = mx.sym.Flatten(data=s3)
    ...
    s4 = mx.sym.BatchNorm(dropout1)
    # stage 5
    fc2 = mx.sym.FullyConnected(name='fc2', data=s4, num_hidden=4096)
    relu7 = mx.sym.Activation(data=fc2, act_type="relu")
    dropout2 = mx.sym.Dropout(data=relu7, p=0.5)
    s5 = mx.sym.BatchNorm(dropout2)
    # stage 6
    fc3 = mx.sym.FullyConnected(name='fc3', data=s5, num_hidden=num_classes)
    final = mx.sym.BatchNorm(fc3)
    if dtype == 'float16':
        fc3 = mx.sym.Cast(data=final, dtype=np.float32)
    softmax = mx.sym.SoftmaxOutput(data=final, name='softmax')
