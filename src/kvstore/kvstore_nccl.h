 ...
 public:
  ...
  virtual ~KVStoreNCCL() {
    for (auto e : nccl_data_) {
      gpuStreamDestroy(e.second.stream);
 ...
 private:
  ...
  void CommSync(const std::vector<T>& dst, int priority) {
    ...
    Engine::Get()->PushSync([this](RunContext rctx) {
        for (auto cur : nccl_data_) {
          CUDA_CALL(gpuSetDevice(cur.second.dev_id));
          CUDA_CALL(gpuStreamSynchronize(cur.second.stream));
        }
      },
      Context::CPU(),
      {},
  ...
  void InitNCCL(const std::vector<Context>& devs) {
    ...
    for (size_t i = 0; i < devs.size(); ++i) {
      ...
      e.comm = comms[i];
      e.rank = i;
      gpuSetDevice(e.dev_id);
      gpuStreamCreate(&(e.stream));
