...
class CuDNNLocalResponseNormOp : public Operator {
 public:
  ...
  ~CuDNNLocalResponseNormOp() {
    if (init_cudnn_) {
      CUDNN_CALL(hipdnnDestroyLRNDescriptor(lrn_desc_));
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    CUDNN_CALL(hipdnnLRNCrossChannelForward(s->dnn_handle_,
                                           lrn_desc_,
                                           HIPDNN_LRN_CROSS_CHANNEL_DIM1,
                                           &alpha,
                                           shape_desc_,
                                           data.dptr_,
                                           &beta,
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    CHECK_EQ(s->dnn_handle_ownership_, mshadow::Stream<gpu>::OwnHandle);
    CUDNN_CALL(hipdnnLRNCrossChannelBackward(s->dnn_handle_,
                                            lrn_desc_,
                                            HIPDNN_LRN_CROSS_CHANNEL_DIM1,
                                            &alpha,
                                            shape_desc_,
 ...
 private:
  inline void Init(mshadow::Stream<gpu> *s,
    ...
    if (!init_cudnn_) {
      ...
      CUDNN_CALL(hipdnnCreateLRNDescriptor(&lrn_desc_));
      CUDNN_CALL(hipdnnSetLRNDescriptor(lrn_desc_,
                                       lrn_n,
                                       alpha,
                                       beta,
                                       lrn_k));
  ...
  hipdnnLRNDescriptor_t lrn_desc_;
