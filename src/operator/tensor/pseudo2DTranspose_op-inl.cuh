...
inline void call_transpose_pseudo2D(index_t dTypeSize, index_t cTypeSize,
  ...
  switch (dTypeSize) {
   case (1): {
    ...
    switch (cTypeSize) {
     case (1):
   hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint8_t, uint8_t>), dim3(grid), dim3(block), 0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
   case (2):
      hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint8_t, uint16_t>),dim3(grid),dim3(block), 0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     case (4):
      hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint8_t, uint32_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     case (8):
 ...
 hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint16_t, uint16_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     case (4):
      
  hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint16_t, uint32_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     case (8):
      
  hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint16_t, uint64_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     default:
      LOG(FATAL) << "Unsupported type combination";
    }
...
hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint32_t, uint32_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     case (8):
     
   hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint32_t, uint64_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     default:
      LOG(FATAL) << "Unsupported type combination";
    }
   ...
   case (8): {
    ...
    switch (cTypeSize) {
     case (8):
hipLaunchKernelGGL((cuda::transpose_pseudo2D<uint64_t, uint64_t>),dim3(grid),dim3(block),0, stream, d_outPtr, d_inpPtr, m, n, nIterY, nIterZ);
      break;
     default:
      LOG(FATAL) << "Unsupported type combination";
    }
  ...
  auto cuErr = hipPeekAtLastError();
  CHECK_EQ(cuErr, hipSuccess) << "Transpose kernel failure: " << hipGetErrorString(cuErr) << ". "
                               << "block: (" << block.x << "," << block.y << "," << block.z << ")"
                               << " grid: (" << grid.x << "," << grid.y << "," << grid.z << ")";
 MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::transpose_pseudo2D);
}
