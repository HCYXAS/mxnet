...
inline void Softmax(Stream<cpu> *s, DType *in, OType *out,
  ...
  for (index_t i = 0; i < static_cast<int>(N); ++i) {
...
__global__ void softmax_compute_kernel(DType *in, OType *out, index_t M, int axis,
  ...
  for (index_t i = x; i < M; i += x_size) {
    smem[x] = ::max(static_cast<DType>(smem[x]), negate ? -in[base + i*sa] : in[base + i*sa]);
  }
  __syncthreads();
...
inline void Softmax(Stream<gpu> *s, DType *in, OType *out,
  ...
  hipLaunchKernelGGL((softmax_compute_kernel<x_bits, OP, negate, AType, ndim>), dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),
...
hipLaunchKernelGGL((softmax_with_length_kernel<x_bits, OP, negate, AType, ndim>),dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),in, out, length, M, axis, sshape, stride, temperature);
  MSHADOW_CUDA_POST_KERNEL_CHECK(softmax_compute_kernel);
}
...
inline void SoftmaxGrad(Stream<gpu> *s, OType *out, OType *ograd,
  ...
  hipLaunchKernelGGL((softmax_gradient_kernel<x_bits, OP1, OP2, Req, negate, AType, ndim>), dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),
...
inline void SoftmaxWithLengthGrad(Stream<gpu> *s, OType *out, OType *ograd,
  ...
  hipLaunchKernelGGL((softmax_with_length_grad_kernel<x_bits, OP1, OP2, Req, negate, AType, ndim>),dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),
