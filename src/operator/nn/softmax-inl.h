...
inline void Softmax(Stream<gpu> *s, DType *in, DType *out,
  ...
  hipLaunchKernelGGL((softmax_compute_kernel<x_bits, OP, negate, DType, ndim>), dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),
...
inline void SoftmaxGrad(Stream<gpu> *s, DType *out, DType *ograd,
  ...
  hipLaunchKernelGGL((softmax_gradient_kernel<x_bits, OP1, OP2, Req, negate, DType, ndim>), dim3(N), dim3(x_size), 0, mshadow::Stream<gpu>::GetStream(s),
