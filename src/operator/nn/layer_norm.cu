...
__global__ void LayerNormFusedForwardKernelContig(const int nbatch,
  ...
  if (bid < nbatch) {
    HIP_DYNAMIC_SHARED(char, buf) //Shared memory
...
void LayerNormGradGPUContig(const LayerNormParam param,
  ...
  if (data_grad_req != kNullOp) {
    MXNET_REAL_ACC_TYPE_SWITCH(in_data.type_flag_, DType, AccType, {
	  ...
	  hipLaunchKernelGGL(HIP_KERNEL_NAME(LayerNormFusedBackwardKernel_Data<LOAD_UNROLL, true, AType>), 
			dim3(data_grid_dim), dim3(data_block_dim), nshared, stream,nbatch, nchannel,
			in_data.dptr<DType>(), out_grad.dptr<DType>(), mean_data.dptr<DType>(),
	                std_data.dptr<DType>(), gamma.dptr<DType>(), data_grad.dptr<DType>());      
} else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(LayerNormFusedBackwardKernel_Data<LOAD_UNROLL, false, AType>),
		     dim3(data_grid_dim), dim3(data_block_dim), nshared, stream, nbatch, nchannel,
		     in_data.dptr<DType>(), out_grad.dptr<DType>(), mean_data.dptr<DType>(),
           	     std_data.dptr<DType>(), gamma.dptr<DType>(), data_grad.dptr<DType>());
	}
