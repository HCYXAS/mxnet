...
#if (MSHADOW_USE_MKL == 1) && defined(_OPENMP) && !defined(__HIPCC__)
 ...
 public:
  ...
  explicit DropoutOp(const DropoutParam &param, Context ctx) {
    ...
    if (ctx.dev_type == kGPU && this->pkeep_ > 0 && !this->cudnn_off_) {
      ...
      CUDNN_CALL(miopenCreateTensorDescriptor(&x_desc_));
      CUDNN_CALL(miopenCreateTensorDescriptor(&y_desc_));
      CUDNN_CALL(miopenCreateTensorDescriptor(&dx_desc_));
      CUDNN_CALL(miopenCreateTensorDescriptor(&dy_desc_));
