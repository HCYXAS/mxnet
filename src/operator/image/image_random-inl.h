...
void ToTensorOpForward(const nnvm::NodeAttrs &attrs,
  ...
  #else
    LOG(FATAL) << "Compile with USE_GPU=1 to use ToTensor operator on GPU.";
  #endif  // MXNET_USE_GPU
...
void NormalizeOpForward(const nnvm::NodeAttrs &attrs,
  ...
  if (std::is_same<xpu, gpu>::value) {
    ...
    #else
      LOG(FATAL) << "Compile with USE_GPU=1 to use Normalize operator on GPU.";
    #endif  // MXNET_USE_GPU
  } else if (inputs[0].ndim() == 3) {
    // 3D input (c, h, w)
    const int length = inputs[0].shape_[1] * inputs[0].shape_[2];
...
void NormalizeOpBackward(const nnvm::NodeAttrs &attrs,
  ...
  if (std::is_same<xpu, gpu>::value) {
    ...
    #else
      LOG(FATAL) << "Compile with USE_GPU=1 to use Normalize backward operator on GPU.";
    #endif  // MXNET_USE_GPU
