...
void ToTensorImplCUDA(mshadow::Stream<gpu> *s,
	...
	hipLaunchKernelGGL(HIP_KERNEL_NAME(ToTensorCudaKernel<gpu, DType>), dim3(blocks), dim3(32, 32), 0, stream,
			   input, output, req, N, H, W, C, normalize_factor);
...
void NormalizeImplCUDA(mshadow::Stream<gpu> *s,
                       ...
                       const float std_d2) {
    hipStream_t stream = mshadow::Stream<gpu>::GetStream(s);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(NormalizeCudaKernel<gpu, DType>), dim3(N), dim3(cuda::kMaxThreadsPerBlock, 1), 0, stream, 
			input, output, req, N, C, H, W, mean_d0, mean_d1, mean_d2,
...
void NormalizeBackwardImplCUDA(mshadow::Stream<gpu> *s,
                               ...
                               const float std_d2) {
    hipStream_t stream = mshadow::Stream<gpu>::GetStream(s);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(NormalizeBackwardCudaKernel<gpu, DType>), dim3(N), dim3(cuda::kMaxThreadsPerBlock, 1), 0, stream,
