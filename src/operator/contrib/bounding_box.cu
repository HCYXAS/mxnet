...
void FilterAndPrepareAuxData(const Tensor<gpu, 3, DType>& data,
  ...
  /*FilterAndPrepareAuxDataKernel<<<blocks,
    ...
    hipLaunchKernelGGL((FilterAndPrepareAuxDataKernel),blocks, n_threads, 0, Stream<gpu>::GetStream(s),data.dptr_, out->dptr_, workspace.scores,
    data.shape_[1], data.shape_[2], N,
    param.valid_thresh, param.id_index,
    param.score_index, param.background_id);
    MSHADOW_CUDA_POST_KERNEL_CHECK(FilterAndPrepareAuxDataKernel);
}
...
void CompactData(const Tensor<gpu, 1, index_t>& indices,
  ...
  if (topk > 0) {
    /*CompactDataKernel<true, check_score><<<blocks, n_threads, 0,
      ...
      score_index, N);*/
hipLaunchKernelGGL((CompactDataKernel<true, check_score>),blocks, n_threads, 0, Stream<gpu>::GetStream(s),indices.dptr_, source.dptr_,
      destination->dptr_, topk,
      source.shape_[2], source.shape_[1],
      score_index, N);
    ...
    /*CompactDataKernel<false, check_score><<<blocks, n_threads, 0,
      ...
      source.shape_[2], source.shape_[1],
      score_index, N);*/
  hipLaunchKernelGGL((CompactDataKernel<false, check_score>),blocks, n_threads, 0, Stream<gpu>::GetStream(s),indices.dptr_, source.dptr_,
      destination->dptr_, topk,
      source.shape_[2], source.shape_[1],
      score_index, N);
  }
...
hipLaunchKernelGGL((CalculateGreedyNMSResultsKernel<box_common_enum::kCorner>),n_blocks, n_threads, 0, Stream<gpu>::GetStream(s),data->dptr_, scratch->dptr_, current_start, n_elems, num_batches,
            num_blocks_per_row_batch, num_blocks_per_row, topk, element_width,
            num_elements_per_batch, param.coord_start,
            param.force_suppress ? -1 : param.id_index,
            param.score_index, param.overlap_thresh);
MSHADOW_CUDA_POST_KERNEL_CHECK(CalculateGreedyNMSResultsKernel<box_common_enum::kCorner>);
      } else {
       /* CalculateGreedyNMSResultsKernel<box_common_enum::kCenter>
          <<<n_blocks, n_threads, 0, Stream<gpu>::GetStream(s)>>>(
            ...
            param.force_suppress ? -1 : param.id_index,
            param.score_index, param.overlap_thresh);*/
hipLaunchKernelGGL((CalculateGreedyNMSResultsKernel<box_common_enum::kCenter>),n_blocks, n_threads, 0, Stream<gpu>::GetStream(s),data->dptr_, scratch->dptr_, current_start, n_elems, num_batches,
            num_blocks_per_row_batch, num_blocks_per_row, topk, element_width,
            num_elements_per_batch, param.coord_start,
            param.force_suppress ? -1 : param.id_index,
            param.score_index, param.overlap_thresh);
MSHADOW_CUDA_POST_KERNEL_CHECK(CalculateGreedyNMSResultsKernel<box_common_enum::kCenter>);
      }
     hipLaunchKernelGGL((ReduceNMSResultTriangleKernel),num_batches, THRESHOLD, 0, Stream<gpu>::GetStream(s),
          scratch->dptr_, data->dptr_, param.score_index,
          element_width, num_batches, num_elements_per_batch,
          current_start, topk);
     MSHADOW_CUDA_POST_KERNEL_CHECK(ReduceNMSResultTriangleKernel);
      ...
      if (n_rest_elems > 0) {
        hipLaunchKernelGGL((ReduceNMSResultRestKernel), num_rest_blocks, n_threads, 0, Stream<gpu>::GetStream(s),
  ...
  MSHADOW_CUDA_POST_KERNEL_CHECK(ReduceNMSResultRestKernel);
      }
    }
  }
};
...
void CompactNMSResults(const Tensor<gpu, 3, DType>& data,
 ...
 hipLaunchKernelGGL((ExtractScoresKernel), n_blocks, n_threads, 0, Stream<gpu>::GetStream(s),
      data.dptr_, scores->dptr_, num_elements, element_width, score_index);
 MSHADOW_CUDA_POST_KERNEL_CHECK(ExtractScoresKernel);
