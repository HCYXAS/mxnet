...
inline void AllFiniteGPU(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(AllFiniteGPUKernel<DType>),dim3(cuda_get_num_blocks(n)),
                       dim3(mshadow::cuda::kBaseThreadNum),0,mshadow::Stream<gpu>::GetStream(s),n,in.dptr_, out.dptr_);
...
inline void MultiAllFiniteGPU(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(MultiAllFiniteGPUKernel<DType>), dim3(cuda_get_num_blocks(param.max_size)),
                                      dim3(mshadow::cuda::kBaseThreadNum), 1,mshadow::Stream<gpu>::GetStream(s),
				     param, out.dptr_);
