...
inline void PSROIPoolForward(const Tensor<gpu, 4, DType> &out,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(PSROIPoolForwardKernel<DType>), dim3(mxnet::op::mxnet_op::cuda_get_num_blocks(count)), dim3(kBaseThreadNum), 0, stream, count, bottom_data,spatial_scale, channels, height, width, pooled_height, pooled_width, bottom_rois, output_dim_, group_size_, top_data);
...
inline void PSROIPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(PSROIPoolBackwardAccKernel<DType>), dim3(mxnet::op::mxnet_op::cuda_get_num_blocks(count)),
