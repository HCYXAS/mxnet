...
inline void MultiBoxTargetForward(const Tensor<gpu, 2, DType> &loc_target,
  ...
  cuda::CheckLaunchParam(init_block_dim, init_thread_dim, "MultiBoxTarget Init");
   ...
   gpuLaunchKernel(GPU_KERNEL_NAME(cuda::InitGroundTruthFlags<DType>), dim3(init_block_dim), dim3(init_thread_dim), 0, 0, gt_flags,
   labels.dptr_, num_batches, num_labels, label_width);
...
MULTIBOX_TARGET_CUDA_CHECK(gpuPeekAtLastError());
  ...
  gpuLaunchKernel(GPU_KERNEL_NAME(cuda::FindBestMatches<DType>), dim3(num_batches), dim3( num_threads), 0, 0, best_matches, gt_flags,   anchor_flags, overlaps, num_anchors, num_labels); 
 MULTIBOX_TARGET_CUDA_CHECK(gpuPeekAtLastError());

  // find good matches with overlap > threshold
  if (overlap_threshold > 0) {
    
      gpuLaunchKernel(GPU_KERNEL_NAME(cuda::FindGoodMatches<DType>), dim3(num_batches), dim3(num_threads), 0, 0, best_matches,
  ...
  if (negative_mining_ratio > 0) {
    ...
    gpuLaunchKernel(GPU_KERNEL_NAME(cuda::NegativeMining<DType>), dim3(num_batches), dim3(num_threads), 0, 0, overlaps,
    cls_preds.dptr_, anchor_flags, buffer, negative_mining_ratio, negative_mining_thresh, minimum_negative_samples, num_anchors,
   ...
   gpuLaunchKernel(GPU_KERNEL_NAME(cuda::UseAllNegatives<DType>), dim3(num_blocks), dim3(num_threads), 0, 0, anchor_flags,
   ...
   gpuLaunchKernel(GPU_KERNEL_NAME(cuda::AssignTrainigTargets<DType>), dim3(num_batches), dim3(num_threads), 0, 0, loc_target.dptr_, 
   loc_mask.dptr_, cls_target.dptr_, anchor_flags, best_matches, labels.dptr_, anchors.dptr_, num_anchors, num_labels, label_width,
