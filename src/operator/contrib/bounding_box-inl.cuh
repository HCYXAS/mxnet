...
void NMSApply(mshadow::Stream<gpu> *s,
  ...
  for (int ref = 0; ref < topk; ref += THRESHOLD) {
    ...
    if (!force_suppress && id_index >= 0) {
       hipLaunchKernelGGL(HIP_KERNEL_NAME(nms_apply_kernel<DType, N, true>),dim3(num_batch), dim3(block_size), 0, stream,
								      topk,
                                                                      sorted_index->dptr_,
                                                                      batch_start->dptr_,
                                                                      buffer->dptr_,
      ...
      if (blocks > 0) {
	hipLaunchKernelGGL(HIP_KERNEL_NAME(nms_apply_kernel_rest<DType, N, true>), dim3(blocks), dim3(block_size), 0, stream,
									topk,
                                                                        sorted_index->dptr_,
                                                                        batch_start->dptr_,
                                                                        buffer->dptr_,
 	...
 	hipLaunchKernelGGL(HIP_KERNEL_NAME(nms_apply_kernel<DType, N, false>), dim3(num_batch), dim3(block_size), 0, stream,
								       topk,
                                                                       sorted_index->dptr_,
                                                                       batch_start->dptr_,
                                                                       buffer->dptr_,
      ...
      int blocks = blocks_per_batch  * num_batch;
      if (blocks > 0) {
       hipLaunchKernelGGL(HIP_KERNEL_NAME(nms_apply_kernel_rest<DType, N, false>), dim3(blocks), dim3(block_size), 0, stream,
									topk,
                                                                        sorted_index->dptr_,
                                                                        batch_start->dptr_,
                                                                        buffer->dptr_,
...
inline void NMSCalculateBatchStart(mshadow::Stream<gpu> *s,
  ...
  hipLaunchKernelGGL(HIP_KERNEL_NAME(nms_calculate_batch_start_kernel), dim3(blocks), dim3(block_size), 0, stream,
								      batch_start->dptr_,
                                                                      valid_batch_id->dptr_,
                                                                      num_elements,num_batch);
