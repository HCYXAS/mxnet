...
//#if CUDA_VERSION >= 9010
  ...
  hipblasHandle_t blas_handle = mshadow::Stream<gpu>::GetBlasHandle(s);
  auto err = HIPBLAS_STATUS_SUCCESS;
  // TODO(cfujitsang): handle computation_precision
  err = hipblasGemmStridedBatchedEx(
      blas_handle, CublasTransposeOp(transA), CublasTransposeOp(transB),
      static_cast<int>(m), static_cast<int>(n), static_cast<int>(k),
      reinterpret_cast<void*>(&alpha),
      a, hipblasType<DType>::kCudaFlag, static_cast<int>(lda), strideA,
      b, hipblasType<DType>::kCudaFlag, static_cast<int>(ldb), strideB,
      reinterpret_cast<void*>(&beta),
      c, hipblasType<DType>::kCudaFlag, static_cast<int>(ldc), strideC,
      static_cast<int>(batchCount), HIPBLAS_R_32F, algo);
  CHECK_EQ(err, HIPBLAS_STATUS_SUCCESS) << "Cublas gemmEx fail.";
#else
...
void gemm_switch_fp32accum(mshadow::Stream<gpu>* s, bool transA, bool transB,
  ...
  if (!(lda & 0x7) && !(ldb & 0x7) && !(ldc & 0x7)) {
    CublasStridedBatchedGemm(s, transA, transB, m, n, k, alpha, a, lda, strideA, b, ldb,
      strideB, beta, c, ldc, strideC, batchCount, HIPBLAS_GEMM_DEFAULT); //TODO CUBLAS_GEMM_DEFAULT_TENSOR_OP
  } else {
    CublasStridedBatchedGemm(s, transA, transB, m, n, k, alpha, a, lda, strideA, b, ldb,
...
void BackwardInterleavedMatMulSelfAttQKGPU(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    if (req[0] == kWriteTo) {
      hipMemsetAsync(queries_keys_values_grads, 0, outputs[0].shape_.Size() * sizeof(DType),
...
void BackwardInterleavedMatMulEncDecQKGPU(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    if (req[1] != kNullOp) {
      if (req[1] == kWriteTo) {
        hipMemsetAsync(keys_values_grads, 0, outputs[1].shape_.Size() * sizeof(DType),
                        mshadow::Stream<gpu>::GetStream(s));
      }
      const float beta = req[1] == kAddTo ? 1.f : 0.f;
      gemm_switch_fp32accum(s,
...
void BackwardInterleavedMatMulEncDecValAttGPU(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
    ...
    if (req[0] != kNullOp) {
      if (req[0] == kWriteTo) {
        hipMemsetAsync(keys_values_grads, 0, outputs[0].shape_.Size() * sizeof(DType),
                        mshadow::Stream<gpu>::GetStream(s));
      }
      const float beta = req[0] == kAddTo ? 1.f : 0.f;
      gemm_switch_fp32accum(s,
