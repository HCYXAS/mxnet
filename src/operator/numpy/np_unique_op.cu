...
void NumpyUniqueGPUNoneAxisImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    int32_t valid_num = 0;
    CUDA_CALL(hipMemcpy(&valid_num, thrust::raw_pointer_cast(&prefix_sum[input_size - 1]),
...
void NumpyUniqueGPUImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    thrust::inclusive_scan(policy, mask.begin(), mask.end(), prefix_sum.begin());
    int32_t valid_num = 0;
    CUDA_CALL(hipMemcpy(&valid_num, thrust::raw_pointer_cast(&prefix_sum[temp_shape[0] - 1]),
                          sizeof(int32_t), hipMemcpyDeviceToHost));
