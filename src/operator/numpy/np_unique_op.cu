...
void NumpyUniqueGPUNoneAxisImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    hipStream_t hip_stream = mshadow::Stream<gpu>::GetStream(stream);
    auto policy = thrust::hip::par.on(stream->stream_);
    ...
    int32_t valid_num = 0;
    CUDA_CALL(hipMemcpyAsync(&valid_num, thrust::raw_pointer_cast(&prefix_sum[input_size - 1]),
...
void NumpyUniqueGPUImpl(const NumpyUniqueParam& param,
  ...
  MXNET_NO_FLOAT16_TYPE_SWITCH(outputs[0].dtype(), DType, {
    ...
    hipStream_t hip_stream = Stream<gpu>::GetStream(stream);
    auto policy = thrust::hip::par.on(stream->stream_);
    ...
    thrust::inclusive_scan(policy, mask.begin(), mask.end(), prefix_sum.begin());
    int32_t valid_num = 0;
    CUDA_CALL(hipMemcpyAsync(&valid_num, thrust::raw_pointer_cast(&prefix_sum[temp_shape[0] - 1]),
                          sizeof(int32_t), hipMemcpyDeviceToHost));
   CUDA_CALL(hipStreamSynchronize(hip_stream));
...
void NumpyUniqueGPUForward(const nnvm::NodeAttrs& attrs,
  ...
  if (inputs[0].shape().ndim() == 0) {
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].dtype(), DType, {
      CUDA_CALL(hipMemcpy(outputs[0].data().dptr<DType>(), inputs[0].data().dptr<DType>(),
                           sizeof(DType), hipMemcpyDeviceToDevice));
    });
    int output_flag = 0;
    if (param.return_index) {
      output_flag += 1;
