...
struct KernelTy {
  ...
  KernelTy(gpufunction _Func, int8_t _ExecutionMode)
...
public:
  ...
  RTLDeviceInfoTy() {
    ...
    err = gpuDeviceGetCount(&NumberOfDevices);
    if (err != gpu_SUCCESS) {
  ...
  ~RTLDeviceInfoTy() {
    ...
    for (auto &module : Modules)
      if (module) {
        gpuresult err = gpuModuleUnload(module);
        if (err != gpu_SUCCESS) {
    ...
    for (auto &ctx : Contexts)
      if (ctx) {
        gpuresult err = gpuCtxDestroy(ctx);
        if (err != gpu_SUCCESS) {
...
int32_t __tgt_rtl_init_device(int32_t device_id) {
  ...
  CUdevice cuDevice;
  DP("Getting device %d\n", device_id);
  gpuresult err = cuDeviceGet(&cuDevice, device_id);
  if (err != gpu_SUCCESS) {
  ...
  err = gpuCtxCreate(&DeviceInfo.Contexts[device_id], CU_CTX_SCHED_BLOCKING_SYNC,
                    cuDevice);
  if (err != gpu_SUCCESS) {
  ...
  gpuError_t error = gpuGetDeviceProperties(&Properties, device_id);
  if (error != gpuSuccess) {
...
__tgt_target_table *__tgt_rtl_load_binary(int32_t device_id,
  ...
  gpuresult err = gpuCtxSetCurrent(DeviceInfo.Contexts[device_id]);
  if (err != gpu_SUCCESS) {
  ...
  err = gpuModuleLoadDataEx(&cumod, image->ImageStart, 0, NULL, NULL);
  if (err != gpu_SUCCESS) {
  ...
  for (__tgt_offload_entry *e = HostBegin; e != HostEnd; ++e) {
    ...
    if (e->size) {
      ...
      gpudeviceptr cuptr;
      size_t cusize;
      err = gpuModuleGetGlobal(&cuptr, &cusize, cumod, e->name);
    ...
    gpufunction fun;
    err = gpuModuleGetFunction(&fun, cumod, e->name);
    ...
    gpudeviceptr ExecModePtr;
    size_t cusize;
    err = gpuModuleGetGlobal(&ExecModePtr, &cusize, cumod, ExecModeName);
    if (err == gpu_SUCCESS) {
      ...
      err = gpuMemcpyDtoH(&ExecModeVal, ExecModePtr, cusize);
      if (err != gpu_SUCCESS) {
...
int32_t __tgt_rtl_data_submit(int32_t device_id, void *tgt_ptr, void *hst_ptr,
  ...
  err = gpuMemcpyHtoD((gpudeviceptr)tgt_ptr, hst_ptr, size);
  if (err != gpu_SUCCESS) {
...
int32_t __tgt_rtl_data_retrieve(int32_t device_id, void *hst_ptr, void *tgt_ptr,
  ...
  err = gpuMemcpyDtoH(hst_ptr, (gpudeviceptr)tgt_ptr, size);
  if (err != gpu_SUCCESS) {
...
int32_t __tgt_rtl_data_delete(int32_t device_id, void *tgt_ptr) {
  ...
  err = gpuMemFree((gpudeviceptr)tgt_ptr);
  if (err != gpu_SUCCESS) {
...
int32_t __tgt_rtl_run_target_team_region(int32_t device_id, void *tgt_entry_ptr,
  ...
  err = gpuLaunchKernel(KernelInfo->Func, cudaBlocksPerGrid, 1, 1,
      cudaThreadsPerBlock, 1, 1, 0 /*bytes of shared memory*/, 0, &args[0], 0);
  if (err != gpu_SUCCESS) {
    DP("Device kernel launch failed!\n");
    CUDA_ERR_STRING(err);
    assert(err == gpu_SUCCESS && "Unable to launch target execution!");
    return OFFLOAD_FAIL;
  }
  ...
  gpuError_t sync_error = gpuDeviceSynchronize();
