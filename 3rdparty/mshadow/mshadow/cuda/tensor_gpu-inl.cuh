...
template<bool clip, int x_bits, typename DType, typename DstPlan, typename SrcPlan1, typename SrcPlan2>
...
inline void AddTakeGrad(Tensor<gpu, 2, DType> dst,
  ...
  if (clip) {
  hipLaunchKernelGGL(HIP_KERNEL_NAME(AddTakeGradKernel<true, kUnitBits, DType>), dim3(dimGrid), dim3(dimBlock), 0, stream, expr::MakePlan(dst),
       expr::MakePlan(index),
       expr::MakePlan(src),
       src.size(0),
       src.size(1), K);
}else {
  hipLaunchKernelGGL(HIP_KERNEL_NAME(AddTakeGradKernel<false, kUnitBits, DType>),
